...
const checkSystemStatusTool = createTool({
  ...
  parameters: z.object({
    component: z.enum(["api", "dashboard", "webhooks", "integrations"]).describe("System component to check"),
  }),
...
const createSupportTicketTool = createTool({
  ...
  handler: async ({ title, description, priority, category }) => {
    ...
    return {
      ...
      category,
      status: "open",
      estimatedResponse: priority === "critical" ? "1 hour" : priority === "high" ? "4 hours" : "24 hours",
      message: "Your support ticket has been created. Our team will respond within the estimated time.",
    };
  },
});
