...
export const createAddMemoriesFn = (inngest: Inngest.Any) => inngest.createFunction(
    { id: 'add-memories' },
    { event: 'app/memories.create' },
    async ({ event }: { event: { data: { statements: string[] } } }) => {
...
export const createUpdateMemoriesFn = (inngest: Inngest.Any) => inngest.createFunction(
    { id: 'update-memories' },
    { event: 'app/memories.update' },
    async ({ event }: { event: { data: { updates: { id: string, statement: string }[] } } }) => {
...
export const createDeleteMemoriesFn = (inngest: Inngest.Any) => inngest.createFunction(
    { id: 'delete-memories' },
    { event: 'app/memories.delete' },
    async ({ event }: { event: { data: { deletions: { id: string, content?: string }[] } } }) => {
...
const recallMemoriesTool = createTool({
    ...
    handler: async ({ queries }, { step, network }) => {
        ...
        if (!searchResults) {
            ...
            network.state.data.retrievedMemories = { memories_found: 0, memories: [] };
            return result;
        }
        ...
        network.state.data.retrievedMemories = result;
        return result;
    },
});
...
const manageMemoriesTool = createTool({
    ...
    handler: async ({ creations, updates, deletions }, { step, network }) => {
        const summary: string[] = [];
        network.state.data.memoriesUpdated = true;
