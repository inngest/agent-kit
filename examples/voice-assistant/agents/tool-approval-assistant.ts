...
import { hitlTools } from '../tools/hitl-backup';
import type { VoiceAssistantNetworkState } from '../index';

/**
 * Backup Tool Approval Assistant - Uses the agent-driven tool approach
 * This agent includes all current HITL functionality for future reference
 */
const toolApprovalAssistant = createAgent<VoiceAssistantNetworkState>({
    name: 'tool-approval-assistant',
    description: 'A backup assistant that uses agent-driven tool approval approach.',
    system: ({ network }) => {
        const now = new Date();
        ...
        let prompt = `
    You are a helpful personal assistant with comprehensive tool approval capabilities.
    ...
    Human-in-the-Loop Guidelines (Agent-Driven Approach):
    ...
    tools: [
        ...
        sendMessage,
        findContact,
        // HITL tools (agent-driven approach)
        ...hitlTools,
        // Final answer tool
