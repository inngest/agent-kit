...
Use `useAgent` for custom implementations requiring fine control over streaming events. For most applications, [`useChat`](/reference/react-hooks/use-chat) is recommended as it provides the same functionality with automatic coordination.
</Card>
...
Object.keys(threads).forEach(threadId => {
  ...
  console.log(`Thread ${threadId}: ${thread.messages.length} messages, status: ${thread.status}`);
});
```
</ParamField>
  ...
  const cleanupOldThreads = () => {
    Object.entries(threads).forEach(([threadId, threadState]) => {
      const daysSinceActivity = 
        (Date.now() - threadState.lastActivity.getTime()) / (1000 * 60 * 60 * 24);
...
function CustomEventHandler() {
  ...
  useEffect(() => {
    ...
    if (currentThread) {
      ...
      if (currentThread.hasNewMessages && currentThread.id !== agent.currentThreadId) {
  ...
  useEffect(() => {
    const cleanup = setInterval(() => {
      ...
      Object.entries(agent.threads).forEach(([threadId, threadState]) => {
        ...
        if (inactiveTime > thirtyMinutes && threadId !== agent.currentThreadId) {
...
function ChatWithRecovery() {
  const { error, connectionError, clearError, clearConnectionError, regenerate } = useAgent({
