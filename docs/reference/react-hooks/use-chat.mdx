...
Use `useChat` for 90% of AgentKit React applications. It handles the complex coordination between real-time events and persistent state automatically.
</Card>
...
<ParamField path="fetchThreads" type="(userId: string, pagination: { limit: number; offset: number }) => Promise<{threads: Thread[]; hasMore: boolean; total: number}>">
...
<ParamField path="createThread" type="(userId: string) => Promise<{threadId: string; title: string}>">
...
<ParamField path="renameThread" type="(threadId: string, title: string) => Promise<void>">
...
const customFetchThreads = async (userId, { limit, offset }) => {
  const response = await fetch(`/api/v2/conversations?user=${userId}&limit=${limit}&offset=${offset}`);
...
```typescript
import { useChat, useMessageActions } from "@inngest/use-agents";
...
```typescript
import { useChat, useConversationBranching } from "@inngest/use-agents";
...
```typescript
import { useChat, useEphemeralThreads } from "@inngest/use-agents";
