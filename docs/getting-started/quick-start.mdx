...
In this tutorial you will create an [Agent](/concepts/agents) and a [Network](/concepts/networks) with AgentKit and run it using the Inngest Dev Server.
...
<Steps>
  ...
  <Step title="Run the server">
    ...
    Next, we'll create an http server in order to run our agent. In the same
...
```
  ...
  <Step title="Test our network">
    ...
    {
      "data": {
        "input": "What is the more performant and secure way to store financial data across multiple organizations?"
      }
    }
    ```

    {/* TODO - this throws an error at the routing agent level */}
