...
const modelWithMaxTokens = anthropic("claude-3-5-haiku-latest", {
  maxTokens: 1000,
});

const modelWithBetaFlags = anthropic("claude-3-5-haiku-latest", {
  betaHeaders: ["prompt-caching-2024-07-31"],
});

const modelWithApiKey = anthropic("claude-3-5-haiku-latest", {
  apiKey: "sk-...",
});
```
