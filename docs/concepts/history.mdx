...
<Note>
  ...
  storage solution by implementing the simple `HistoryConfig` interface.
</Note>
...
- **Purpose**: Saves new messages from the current turn to your database
...
Here's an example of a history adapter using Inngest's durable `step` function:
...
const postgresHistoryAdapter: HistoryConfig<any> = {
  ...
  get: async ({ step, threadId }) => {
    ...
    const messages = await step.run("fetch-history-from-db", async () => {
      ...
      // Transform your database records into AgentResult[]
...
// Helper function to transform DB records to AgentResults
function transformToAgentResults(dbMessages: any[]): AgentResult[] {
  // Implementation depends on your database schema
  return dbMessages.map((msg) => {
    // Transform your database record into AgentResult format
    // This is specific to your schema
  });
}
```
...
The client sends a message with a `threadId`. AgentKit automatically loads the full conversation context from your database before the agent runs.
...
await agent.run("Continue our conversation", { state });
// AgentKit calls history.get() to load full context
```
...
await agent.run("New message", { state });
