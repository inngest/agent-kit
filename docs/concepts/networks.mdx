...
A Network must specify a  `router` that will be used to determine the next Agent to run.

A `router` can be a [Routing Agent](/concepts/routers#routing-agent) or a [Router Function](/concepts/routers#router-function).
...
### Combining `maxIter` and `router`

You can combine `maxIter` and `router` to create a Network that will stop after a certain number of iterations or when a condition is met.

However, please note that the `maxIter` option can prevent the `router` from being called (For example, if `maxIter` is set to 1, the `router` will only be called once).
