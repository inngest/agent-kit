...
Networks are **Systems of [Agents](/concepts/agents)**. Use Networks to create powerful AI Agents by combining multiple Agents.
...
A Network can provide a default model for Agents that don't have one:
...
The `defaultModel` is only used by the network when no model is provided on an Agent.
This means that a network can end up using multiple models if some Agents relies on different models:
