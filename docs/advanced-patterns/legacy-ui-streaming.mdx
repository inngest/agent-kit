---
title: UI Streaming with useAgent
description: "Stream AgentKit events to your UI with the useAgent hook."
---
...
```ts title="inngest/functions/run-agent-chat.ts"
...
<ParamField path="error" type="{ message: string; ... } | undefined">
...
<ParamField path="connectionError" type="{ message: string; ... } | undefined">
...
<ParamField
  path="sendMessageToThread"
  type="(threadId: string, message: string) => Promise<void>"
>
...
<ParamField
  path="replaceMessages"
  type="(messages: ConversationMessage[]) => void"
>
...
_(For brevity, other part types like `DataUIPart`, `ReasoningUIPart`, etc., are omitted here but follow a similar structure. You can find their full definitions in the `use-agent.ts` file in the example.)_
