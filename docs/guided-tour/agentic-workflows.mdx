...
AgentKit's Router enables us to implement such dynamic workflow with code by providing a `router` function:
...
The provided Router function (`router`) receives a `network` argument granting access to the Network's state and Agents.
