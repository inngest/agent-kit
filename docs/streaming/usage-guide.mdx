---
title: "Usage Guide"
description: "A deep dive into streaming agents"
---
...
<Steps>
  <Step title="Inngest Client - /api/inngest/client.ts">
...
```
    ...
    <Step title="Realtime Channel - /api/inngest/realtime.ts">
...
```
    ...
    <Step title="Chat API Route - /api/chat/route.ts">
...
```
    ...
    <Step title="Inngest Route - /api/inngest/route.ts">
...
Let’s start by creating our event matcher and query writer agents. Each agent will have access to only one tool each which we will ensure is always invoked by defining a static tool_choice.
...
By following this guide and using the `useAgents` hook, you now have:
