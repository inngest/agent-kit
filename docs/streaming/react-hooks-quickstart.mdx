...
<CardGroup>
  ...
  <Card
    title="Live Demo"
    href="https://agent-kit-demo.vercel.app"
    icon="globe"
  >
    ...
    const result = await network.run(message, {
      streaming: {
        publish: async (chunk) => {
          ...
          await publish(userChannel(userId).agent_stream({
            ...chunk,
...
🎉 **That's it!** You now have a fully functional streaming chat app with:
...
<button onClick={() => likeMessage(message.id)}>👍</button>
```
...
<CardGroup>
  <Card
    ...
    href="/streaming/hook-selection-guide"
  >
