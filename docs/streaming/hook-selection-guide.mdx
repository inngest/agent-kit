...
const {
  messages, sendMessage, status,     // Real-time streaming
  threads, switchToThread,           // Thread management
  createNewThread, deleteThread     // Thread operations
} = useChat({
...
const {
  ...
  threads, setCurrentThread,        // Manual thread switching
  isConnected, connectionError,     // Connection management
...
```typescript
const { 
  editingMessage, editValue, setEditValue,
