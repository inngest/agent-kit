...
const safeParseOpenAIJson = (str: string): any => {
...
export const requestParser: AgenticModel.RequestParser<OpenAi.AiModel> = (
  ...
  const request: AiAdapter.Input<OpenAi.AiModel> = {
    messages: messages.map((m) => {
      ...
      return {
        ...(m.stop_reason ? { finish_reason: StateStopReasonToOpenAiStopReason[m.stop_reason] } : {}),
        role,
        content: m.content,
        ...
        tool_calls: role === 'assistant' && !!m.tools ? m.tools?.map((tool) => ({
          id: tool.id,
          type: "function",
          function: {
            name: tool.name,
    ...
    (acc, choice) => {
      ...
      return [
        ...acc,
        {
          ...
          // NOTE: this is very ugly, we need to better handle different shape of messages
          // TODO: refactor + unit tests
          // @ts-expect-error TODO: patch `inngest` to support `finish_reason`
          ...(choice.finish_reason ? { stop_reason: OpenAiStopReasonToStateStopReason[choice.finish_reason] } : {}),
