...
**AgentKit is a TypeScript framework for building multi-agent networks with deterministic routing and rich tooling via MCP (Model Context Protocol).** Combine agents with prompts, tools, and MCP servers to create collaborative workflows with shared state. Works with multiple model providers (Anthropic, OpenAI, etc.), supports code-based and LLM-based orchestration, and offers built-in tracing for debugging and optimization.

### Core Concepts

| Concept | Description |
|---------|-------------|
| **Agents** | LLM calls combined with prompts, [tools](https://agentkit.inngest.com/concepts/tools), and [MCP](https://agentkit.inngest.com/advanced-patterns/mcp) servers |
| **Networks** | Agent collaboration with shared State, including handoff between agents |
| **State** | Conversation history + typed state machine for routing decisions |
| **Routers** | Code-based or LLM-based (e.g., ReAct) orchestration logic |
| **Tracing** | Built-in debugging and optimization locally and in cloud |
...
**Note:** AgentKit v0.9.0+ requires `inngest` as separate dependency alongside `@inngest/agent-kit`.
...
const server = createServer({ agents: [myAgent] });
server.listen(3000);
```
...
| Requirement | Details |
|-------------|---------|
| **Node.js** | 18+ (TypeScript AI development) |
| **Model Provider** | Anthropic (Claude), OpenAI, or other LLM API key |
| **MCP Servers** | Optional: Smithery servers or self-hosted MCP for tools |
| **Inngest** | Required runtime dependency (v0.9.0+) |

### Key Features

| Feature | Description |
|---------|-------------|
| **Deterministic Routing** | Code-based routers for predictable agent handoffs |
| **LLM-Based Orchestration** | ReAct and other LLM-driven routing patterns |
| **MCP Integration** | Rich tooling via Model Context Protocol servers |
| **Multi-Provider Support** | Anthropic, OpenAI, and more model providers |
| **Fault-Tolerant** | Inngest orchestration engine for cloud reliability |
| **Built-in Tracing** | Debug and optimize workflows locally and in production |

### License

AgentKit is open source. Check the repository for license details.

### Help & Resources

| Resource | Link |
|----------|------|
| **Documentation** | [agentkit.inngest.com/overview](https://agentkit.inngest.com/overview) |
| **Examples** | [agentkit.inngest.com/examples](https://agentkit.inngest.com/examples/overview) |
| **Blog** | [inngest.com/blog](https://www.inngest.com/blog?ref=github-agent-kit-readme) |
| **Discord** | [inngest.com/discord](https://www.inngest.com/discord) |
| **Issues** | [GitHub Issues](https://github.com/inngest/agent-kit/issues) |
