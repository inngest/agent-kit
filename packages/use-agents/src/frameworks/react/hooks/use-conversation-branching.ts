...
export function useConversationBranching({
  ...
  const logger = useMemo(
    () => createDebugLogger("useConversationBranching", debug),
    [debug]
  );
  ...
  const [branchedThreads, setBranchedThreads] = useState<
    Map<string, BranchedThread>
  >(new Map());
  ...
  const saveToStorage = useCallback(
    (threads: Map<string, BranchedThread>) => {
      if (!storage) return;
      try {
  ...
  const getOrCreateBranchedThread = useCallback(
    (threadId: string): BranchedThread => {
  ...
  const createBranchFromMessage = useCallback(
    (
    ...
    ): { branchId: string; branchMessages: ConversationMessage[] } => {
      ...
      const branchPointIndex = currentMessages.findIndex(
        (m) => m.id === messageId
      );
      if (branchPointIndex === -1) {
        throw new Error(
          `Message ${messageId} not found in current conversation`
        );
      }
  ...
  const switchToBranch = useCallback(
    (threadId: string, branchId: string): ConversationMessage[] => {
      const branchedThread = getOrCreateBranchedThread(threadId);
      const targetBranch =
        branchId === "main"
      ...
      const updatedBranchedThread: BranchedThread = {
        ...
        branchHistory: [
          ...branchedThread.branchHistory.filter((id) => id !== branchId),
          branchId,
        ], // Move to end
      };
  ...
  const addMessageToBranch = useCallback(
    (threadId: string, message: ConversationMessage): void => {
      const branchedThread = getOrCreateBranchedThread(threadId);
      const activeBranch =
        branchedThread.branches.find(
          (b) => b.id === branchedThread.activeBranchId
        ) || branchedThread.mainBranch;
      ...
      const updatedBranchedThread: BranchedThread = {
        ...
        ...(branchedThread.activeBranchId === "main"
          ...
          : {
              branches: branchedThread.branches.map((b) =>
                b.id === branchedThread.activeBranchId ? updatedBranch : b
              ),
            }),
      };
  ...
  const getCurrentBranchMessages = useCallback(
    (threadId: string): ConversationMessage[] => {
  ...
  const getBranchInfo = useCallback(
    (threadId: string) => {
      ...
      const currentIndex = branchedThread.branchHistory.indexOf(
        branchedThread.activeBranchId
      );
      return {
  ...
  const goBack = useCallback(
    (threadId: string): ConversationMessage[] | null => {
      const branchedThread = branchedThreads.get(threadId);
  ...
  const branchingSendMessage = useCallback(
    async (
      originalSendMessage: (
        message: string,
      ...
      if (options?.editFromMessageId) {
        ...
        const { branchId: newBranchId, branchMessages } =
          createBranchFromMessage(
            threadId,
            options.editFromMessageId,
            currentMessages
          );
        ...
        const originalMessage = currentMessages.find(
          (m) => m.id === options.editFromMessageId
        );
        const originalClientState = originalMessage?.clientState || {};
        const editIndex = currentMessages.findIndex(
          (m) => m.id === options.editFromMessageId
        );
        ...
        await sendMessageToThread(threadId, message, {
          ...
          state: () => {
            ...
            const formattedHistory =
              formatMessagesToAgentKitHistory(branchHistory);
  ...
  const branchingReplaceMessages = useCallback(
  ...
  const branchingClearMessages = useCallback(
    (threadId: string) => {
