import type { IClientTransport, SendMessageParams, RequestOptions, FetchThreadsParams, FetchHistoryParams, CreateThreadParams, DeleteThreadParams, ApproveToolCallParams, GetRealtimeTokenParams } from "../ports/transport.js";
import type { Thread } from "../../types/index.js";
import { DefaultHttpTransport, type DefaultHttpTransportConfig } from "./http-transport";
...
export class InMemorySessionTransport implements IClientTransport {
  ...
  async getRealtimeToken(params: GetRealtimeTokenParams, options?: RequestOptions): Promise<any> {
    return this.http.getRealtimeToken(params, options);
  }

  // Ephemeral, per-tab thread list
  async fetchThreads(params: FetchThreadsParams): Promise<{ threads: Thread[]; hasMore: boolean; total: number; nextCursorTimestamp?: string | null; nextCursorId?: string | null }>{
    const key = getUserKey(params.userId);
    const list = threadsByUser.get(key) || [];
    const limit = params.limit ?? 20;
    const offset = params.offset ?? 0;
    const slice = list.slice(offset, offset + limit);
    return { threads: slice, hasMore: offset + limit < list.length, total: list.length };
  }
  ...
  async createThread(params: CreateThreadParams): Promise<{ threadId: string; title: string }>{
    const id = crypto.randomUUID?.() || String(Date.now());
    const now = new Date();
    const thread: Thread = { id, title: params.title || "New conversation", messageCount: 0, lastMessageAt: now, createdAt: now, updatedAt: now } as Thread;
    const key = getUserKey(params.userId);
    const list = threadsByUser.get(key) || [];
  ...
  async deleteThread(params: DeleteThreadParams): Promise<void> {
    for (const [key, list] of threadsByUser) {
      threadsByUser.set(key, list.filter((t) => t.id !== params.threadId));
    }
  }
...
export function createInMemorySessionTransport(config?: Partial<DefaultHttpTransportConfig>): InMemorySessionTransport {
