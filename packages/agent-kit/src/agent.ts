...
export class Agent<T extends StateData> {
  ...
  private shouldIncludeTool(
    ...
    if (this.mcpConfig?.includeTools && this.mcpConfig.includeTools.length > 0) {
    ...
    if (this.mcpConfig?.excludeTools && this.mcpConfig.excludeTools.length > 0) {
      const isExcluded = this.matchesFilter(toolName, this.mcpConfig.excludeTools);
